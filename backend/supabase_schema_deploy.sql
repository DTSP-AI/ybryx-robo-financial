...
-- Modified for deployment without pgvector (embeddings stored as JSONB)
...
-- Note: pgvector disabled for now - embeddings stored as JSONB arrays
-- To enable later: CREATE EXTENSION IF NOT EXISTS vector;

-- ============================================================================
-- CORE TABLES (modify existing users table if needed)
-- ============================================================================
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'role') THEN
        ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'user' CHECK (role IN ('user', 'agent', 'admin', 'system'));
    END IF;
...
COMMENT ON TABLE goal_assessments IS 'Agent goal tracking with embeddings stored as JSONB';
...
-- Note: pgvector extension not enabled - embeddings stored as JSONB arrays
-- To enable pgvector later:
-- 1. Install pgvector extension in PostgreSQL
-- 2. Run: CREATE EXTENSION IF NOT EXISTS vector;
-- 3. Alter goal_vector and belief_vector columns from JSONB to VECTOR(1536)
-- 4. Add vector similarity indexes
